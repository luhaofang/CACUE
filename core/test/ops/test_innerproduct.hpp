...
TEST_CASE("inner_product")
{
	...
	SECTION("ip data vs validate data."){
		...
		blob_tools::read_data2blob(b,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/feature_map.txt");
		...
		blob_tools::read_data2blob(validate_,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/result.txt");

		blob *output = (blob*)op->out_data();

#if __PARALLELTYPE__ == __GPU__
		cacu_saxpby(output->s_data(),(float_t)-1.0,validate_->s_data(),(float_t)1.0,validate_->count());
...
TEST_CASE("inner_product_multidata")
{
	...
	SECTION("ip data vs validate data."){
		...
		for(int i = 0 ; i< 100; ++i)
			blob_tools::read_data2blob(b,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/feature_map.txt",i);
		...
		for(int i = 0 ; i< 100; ++i)
			blob_tools::read_data2blob(validate_,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/result.txt",i);
...
TEST_CASE("inner_product_grad")
{
	...
	SECTION("ip grad data vs validate data."){
		...
		blob_tools::read_data2blob(b,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/feature_map_bp.txt");
		...
		args *args_ = new args(64,0,0,0,0,0);
		inner_product_op *op = (inner_product_op *)operator_factory::create_op(CACU_INNERPRODUCT,input,args_);
		blob_tools::read_data2blob(op->get_weight(0),"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/w_bp.txt");
		...
		blob_tools::read_data2blob(validate_,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/result_bp.txt");

		blob *validate_k = cacu_allocator::create_blob(64, 512, 7, 7,test);
		blob_tools::read_data2blob(validate_k,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/fgrad_bp.txt");
		blob *validate_b = cacu_allocator::create_blob(64, 1, 1, 1,test);
		blob_tools::read_data2blob(validate_b,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/bgrad_bp.txt");

#if __PARALLELTYPE__ == __GPU__

		cacu_saxpby(b->s_diff(),(float_t)-1.0,validate_->s_data(),(float_t)1.0,validate_->count());
		vec_t test(validate_->count());
		cuda_copy2host(&test[0],validate_->s_data(),test.size());
		float_t acc_error = 0.0;
		for(int i = 0 ; i < test.size(); ++i){
		...
		cacu_saxpby(op->get_weight(0)->s_diff(),(float_t)-1.0,validate_k->s_data(),(float_t)1.0,validate_k->count());
		...
		cacu_saxpby(op->get_weight(1)->s_diff(),(float_t)-1.0,validate_b->s_data(),(float_t)1.0,validate_b->count());
...
TEST_CASE("inner_product_grad_multidata")
{
	...
	SECTION("ip grad data vs validate data."){
		...
		for(int i = 0 ; i< 100; ++i)
			blob_tools::read_data2blob(b,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/feature_map_bp.txt",i);
		...
		for(int i = 0 ; i< 100; ++i)
			blob_tools::read_data2blob(validate_,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/result_bp.txt",i);
		...
		cacu_scalex(op->get_weight(0)->s_diff(),op->get_weight(0)->count(), (float_t)1/100);
		cacu_saxpby(op->get_weight(0)->s_diff(),(float_t)-1.0,validate_k->s_data(),(float_t)1.0,validate_k->count());
		test.resize(validate_k->count());
		cuda_copy2host(&test[0],validate_k->s_data(),test.size());
		acc_error = 0.0;
		...
		cacu_scalex(op->get_weight(1)->s_diff(),op->get_weight(1)->count(), (float_t)1/100);
		cacu_saxpby(op->get_weight(1)->s_diff(),(float_t)-1.0,validate_b->s_data(),(float_t)1.0,validate_b->count());
		test.resize(validate_b->count());
		cuda_copy2host(&test[0],validate_b->s_data(),test.size());
		acc_error = 0.0;
