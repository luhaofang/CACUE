...
#endif
		blob *b = cacu_allocator::create_blob(1, 3, 24, 24,test);
...
TEST_CASE("inner_product_multidata")
{
	...
	SECTION("ip data vs validate data."){
		...
		cublasCreate_v2(&handle);
#endif
		blob *b = cacu_allocator::create_blob(100, 3, 24, 24,test);
		for(int i = 0 ; i< 100; ++i)
...
TEST_CASE("inner_product_grad")
{
	...
	SECTION("ip grad data vs validate data."){
		...
		cublasCreate_v2(&handle);
#endif
		blob *b = cacu_allocator::create_blob(1, 3, 24, 24,train);
		blob_ops::read_data2blob(b,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/feature_map.txt");
		...
		blob *validate_ = cacu_allocator::create_blob(1, 3, 24, 24,test);
		blob_ops::read_data2blob(validate_,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/conv_grad_result.txt");

		blob *validate_k = cacu_allocator::create_blob(1000, 3, 24, 24,test);
...
TEST_CASE("inner_product_grad_multidata")
{
	...
	SECTION("ip grad data vs validate data."){
		...
		cublasCreate_v2(&handle);
#endif
		blob *b = cacu_allocator::create_blob(100, 3, 24, 24,train);
		for(int i = 0 ; i< 100; ++i)
			blob_ops::read_data2blob(b,"/home/seal/cuda-workspace/CACUE/core/test/python/innerproduct/feature_map.txt",i);
		...
		blob *validate_ = cacu_allocator::create_blob(100, 3, 24, 24,test);
		for(int i = 0 ; i< 100; ++i)
