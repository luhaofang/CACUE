...
bool JpegEncoder::encodeToDCT(float *pdata_, int quality_scale)
{
	...
	vector<char> yData(64);
	vector<char> cbData(64);
	vector<char> crData(64);
	vector<short> yQuant(64);
	vector<short> cbQuant(64);
	vector<short> crQuant(64);
	...
	for(int yPos=0; yPos<m_height; yPos+=8)
	{
		for (int xPos=0; xPos<m_width; xPos+=8)
		{
			...
			_convertColorSpace(xPos, yPos, &yData[0], &cbData[0], &crData[0]);

			_foword_FDC(&yData[0], &yQuant[0]);

			_foword_FDC(&cbData[0], &cbQuant[0]);

			_foword_FDC(&crData[0], &crQuant[0]);

			for (int y = 0; y < 8; ++y)
				for(int x = 0; x < 8; ++x){
					pdata_[(yPos * 8 + y) * m_width * 8 + xPos * 8 + x] = (float_t)(yQuant[y * 8 + x]);
					pdata_[(yPos * 8 + y) * m_width * 8 + xPos * 8 + x + m_height * 8 * m_width * 8] = (float_t)(cbQuant[y * 8 + x]);
					pdata_[(yPos * 8 + y) * m_width * 8 + xPos * 8 + x + m_height * 8 * m_width * 8 * 2] = (float_t)(crQuant[y * 8 + x]);
				}
		}
	}
	return true;
...
bool JpegEncoder::encodeToY(float *pdata_, int quality_scale){
	if(m_rgbBuffer==0 || m_width==0 || m_height==0) return false;
	...
	for(int yPos=0; yPos<m_height; yPos+=8)
	{
		for (int xPos=0; xPos<m_width; xPos+=8)
		{
			...
			for (int y = 0; y < 8; ++y)
				for(int x = 0; x < 8; ++x){
					pdata_[(yPos * 8 + y) * m_width * 8 + xPos * 8 + x] = (float_t)(yData[y * 8 + x]);
				}
		}
	}
	return true;
...
void JpegEncoder::_initQualityTables(int quality_scale)
{
	...
	for(int i=0; i<64; i++)
	{
		...
		m_YTable[int(ZigZag[i])] = (unsigned char)temp;

		temp = ((int)(Chrominance_Quantization_Table[i] * quality_scale + 50) / 100);
		if (temp<=0) 	temp = 1;
		if (temp>0xFF) temp = 0xFF;
		m_CbCrTable[int(ZigZag[i])] = (unsigned char)temp;
	}
}
...
void JpegEncoder::_foword_FDC(const char* channel_data, short* fdc_data)
{
	...
	for(int v=0; v<8; v++)
	{
		for(int u=0; u<8; u++)
		{
			...
			temp *= alpha_u*alpha_v/m_YTable[int(ZigZag[v*8+u])];
			fdc_data[int(ZigZag[v*8+u])] = (short) ((short)(temp + 16384.5) - 16384);
		}
	}
}
